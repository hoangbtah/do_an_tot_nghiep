<template >   
    <!-- form xác nhận xóa -->
<div class="m-dialog" v-if="isShowDialogDeleteCatagory">
   <div class="m-dialog-confirm-del-content">
       <div class="m-dialog-confirm-del-header">
           <!-- <div class="m-icon-confirm-delete"></div> -->
           <div class="infor-warning"><label>Bạn có chắc muốn xóa danh mục này không ?</label></div>
       </div>
       <div class="m-dialog-confirm-del-btn">
           ...
           <div class="m-dialog-confirm-del-yes">
               <button id="m-dialog-confirm-del-yes" class="m-btn" @click="btnDelProductDiscount(catagory)">Có</button>
           </div>
       </div>
   </div>
</div>
...
Vue.use(Toasted);
export default {
  name: "DialogDeleteCatagory",
  created() {
    //    this.getProduct();
  },
  computed: {
    ...mapGetters(["catagory", "isShowDialogDeleteCatagory"])
  },
  methods: {
    ...
    async btnDelProductDiscount(catagory) {
      try {
        await axios.delete(
          `https://localhost:7159/api/v1/Catagory/${catagory.catagoryId}`
        );
        ...
        // ấn dialog đi
        this.$store.commit("TOGGLE_ISSHOW_DIALOG_DELETE_CATAGORY");
        this.$store.commit("DELETE_CATAGORY", catagory.catagoryId);
      } catch (error) {
       
        // Hiển thị thông báo thành công
        this.$toasted.show(
          "Cảnh báo! Xóa danh mục này có thể ảnh hưởng đến những sản phẩm đang ở trong danh mục này, hãy chắc chăn rằng không có sản phẩm nào thuộc danh mục này",
          {
